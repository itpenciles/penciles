...
export const analyzePropertyWithGemini = async (inputType: 'url' | 'address' | 'coords' | 'location', value: string): Promise<Property> => {
    ...
    for (const model of modelsToTry) {
        try {
            ...
            // FIX: Removed `responseMimeType` and `responseSchema` because they are not supported when using tools like `googleSearch`.
            ...
            // FIX: The response might be wrapped in markdown JSON ticks, so we clean it before parsing.
    ...
    // FIX: Provide more specific error messages to the user instead of a generic failure.
