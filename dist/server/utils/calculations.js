...
export const calculateMetrics = (financials) => {
    const { purchasePrice, rehabCost, downPaymentPercent, monthlyRents, vacancyRate, maintenanceRate, managementRate, capexRate, monthlyTaxes, monthlyInsurance, monthlyWaterSewer, monthlyStreetLights, monthlyGas, monthlyElectric, monthlyLandscaping, monthlyHoaFee, operatingMiscFee, loanInterestRate, loanTermYears, originationFeePercent, closingFee, processingFee, appraisalFee, titleFee, brokerAgentFee, homeWarrantyFee, attorneyFee, closingMiscFee, sellerCreditTax, sellerCreditSewer, sellerCreditOrigination, sellerCreditClosing, sellerCreditRents, sellerCreditSecurityDeposit, sellerCreditMisc } = financials;
...
export const calculateWholesaleMetrics = (inputs) => {
...
export const calculateSubjectToMetrics = (inputs) => {
...
export const calculateSellerFinancingMetrics = (inputs) => {
...
export const calculateBrrrrMetrics = (inputs) => {
    const { purchasePrice, arv, purchaseCosts, rehabCosts, financing, refinance, expenses, monthlyRent, holdingCostsMonthly } = inputs;
    // Helper to sum object values
    const sumValues = (obj) => Object.values(obj || {}).reduce((a, b) => (Number(a) || 0) + (Number(b) || 0), 0);
    ...
    // Operating Expenses
    const { monthlyTaxes, monthlyInsurance, monthlyHoa, monthlyWaterSewer, monthlyStreetLights, monthlyGas, monthlyElectric, monthlyLandscaping, monthlyMiscFees, vacancyRate, maintenanceRate, capexRate, managementRate, otherMonthlyIncome } = expenses || {};
