...
const getClientIdSnippet = (clientId) => {
...
export const handleGoogleLogin = async (req, res) => {
    ...
    try {
        ...
        if (isFreeTier) {
            try {
                const countResult = await query(`SELECT COUNT(*) FROM properties WHERE user_id = $1`, [dbUser.id]);
                ...
                if (parseInt(dbUser.analysis_count) !== actualCount) {
                    ...
                    const updateCountRes = await query('UPDATE users SET analysis_count = $1 WHERE id = $2 RETURNING analysis_count', [actualCount, dbUser.id]);
