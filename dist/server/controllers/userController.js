...
export const updateUserSubscription = async (req, res) => {
    ...
    try {
        ...
        const planMap = new Map(plansRes.rows.map((r) => [r.key, r.monthly_price]));
        if (planMap.has(oldTier))
            oldPrice = Number(planMap.get(oldTier));
        if (planMap.has(tier))
            newPrice = Number(planMap.get(tier));
        ...
        if (oldTier === 'Free' && tier !== 'Free')
            ...
            changeType = 'upgrade';
        else if (newPrice < oldPrice && tier !== 'Free')
        ...
        if (tier === 'PayAsYouGo') {
            ...
            result = await query('UPDATE users SET subscription_tier = $1, updated_at = now(), analysis_count = $2, analysis_limit_reset_at = NULL, credits = credits + $3 WHERE id = $4 RETURNING id, name, email, profile_picture_url, subscription_tier, analysis_count, analysis_limit_reset_at, role, credits', [tier, totalHistoricalCount, PAYG_RETAINER, userId]);
        }
            ...
            result = await query('UPDATE users SET subscription_tier = $1, updated_at = now(), analysis_count = $2, analysis_limit_reset_at = $3 WHERE id = $4 RETURNING id, name, email, profile_picture_url, subscription_tier, analysis_count, analysis_limit_reset_at, role, credits', [tier, totalHistoricalCount, newResetDate, userId]);
        }
        else {
            // For 'Free' plan (downgrade or cancel)
            result = await query('UPDATE users SET subscription_tier = $1, updated_at = now(), analysis_count = $2, analysis_limit_reset_at = NULL WHERE id = $3 RETURNING id, name, email, profile_picture_url, subscription_tier, analysis_count, analysis_limit_reset_at, role, credits', [tier, totalHistoricalCount, userId]);
        }
        // Log to history
        if (oldTier !== tier || tier === 'PayAsYouGo') {
            await query(`INSERT INTO subscription_history (user_id, old_tier, new_tier, change_type, amount) VALUES ($1, $2, $3, $4, $5)`, [userId, oldTier, tier, changeType, revenueImpact]);
        }
...
export const purchaseCredits = async (req, res) => {
    ...
    try {
        // Update credits
        const result = await query('UPDATE users SET credits = credits + $1, updated_at = now() WHERE id = $2 RETURNING credits', [amount, userId]);
        // Log transaction
        await query(`INSERT INTO subscription_history (user_id, old_tier, new_tier, change_type, amount) VALUES ($1, 'PayAsYouGo', 'PayAsYouGo', 'credit_purchase', $2)`, [userId, amount]);
...
export const trackUserAction = async (req, res) => {
    const userId = req.user?.id;
