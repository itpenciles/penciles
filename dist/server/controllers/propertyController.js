...
export const getProperties = async (req, res) => {
    ...
    try {
        ...
        const result = await query('SELECT id, property_data, created_at FROM properties WHERE user_id = $1 ORDER BY created_at DESC', [userId]);
...
export const addProperty = async (req, res) => {
    ...
    try {
        ...
        const result = await query('INSERT INTO properties (user_id, property_data) VALUES ($1, $2) RETURNING id, property_data, created_at', [userId, propertyData]);
...
export const updateProperty = async (req, res) => {
    ...
    try {
        ...
        const skipReevaluation = req.body.skipReevaluation === true;
        ...
        if (!skipReevaluation) {
            try {
                ...
                const newRecommendation = await reevaluatePropertyWithGemini(propertyData, strategyToAnalyze);
        ...
        delete updatedDataToStore.skipReevaluation;
        // Step 3: Save the fully updated object to the database.
        const result = await query('UPDATE properties SET property_data = $1, updated_at = now() WHERE id = $2 AND user_id = $3 RETURNING id, property_data, created_at', [updatedDataToStore, id, userId]);
...
export const deleteProperty = async (req, res) => {
    ...
    try {
        ...
        const selectResult = await query('SELECT property_data FROM properties WHERE id = $1 AND user_id = $2', [id, userId]);
        ...
        await query('UPDATE properties SET property_data = $1, updated_at = now() WHERE id = $2 AND user_id = $3', [softDeletedData, id, userId]);
