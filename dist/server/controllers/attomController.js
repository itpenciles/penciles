...
export const getComparables = async (req, res) => {
    try {
        if (!ATTOM_API_KEY) {
        ...
        const { address, distance, recency, sqft, bedrooms, bathrooms } = req.body;
        if (!address) {
        ...
        if (bedrooms) {
            if (bedrooms === 'Same')
                params.bedroomsRange = 0;
            else if (bedrooms === '+-1')
                params.bedroomsRange = 1;
            else if (bedrooms === '+-2')
                params.bedroomsRange = 2;
        }
        if (bathrooms) {
            if (bathrooms === 'Same')
                params.bathroomRange = 0;
            else if (bathrooms === '+-1')
                params.bathroomRange = 1;
        }
        if (sqft) {
            // We don't know the subject sqft here to calculate %, so we use reasonable defaults
            if (sqft === 'Same')
                params.sqFeetRange = 100;
            else if (sqft === '+-10%')
                params.sqFeetRange = 300;
            else if (sqft === '+-20%')
                params.sqFeetRange = 600;
        }
        ...
        const comparables = responseData
            .filter((p) => p.PRODUCT_INFO_ext?.['@Product_ext'] === 'SalesCompProperties')
            .map((p) => {
        ...
        const apiErrorMessage = error.response?.data?.RESPONSE_GROUP?.PRODUCT?.STATUS?.['@_Description'] ||
...
function getRecencyMonths(recency) {
    switch (recency) {
