...
export const getUserDetail = async (req, res) => {
    ...
    try {
        ...
        const billingHistory = historyRes.rows.map((row) => ({
            id: row.id,
        ...
        const startRow = historyRes.rows.slice().reverse().find((r) => r.change_type === 'new' || r.change_type === 'upgrade');
...
export const cancelUserSubscription = async (req, res) => {
    ...
    try {
        ...
        await query('UPDATE users SET subscription_tier = $1, updated_at = now(), analysis_limit_reset_at = NULL WHERE id = $2', ['Free', id]);
        await query(`INSERT INTO subscription_history (user_id, old_tier, new_tier, change_type, amount) VALUES ($1, $2, $3, $4, $5)`, [id, oldTier, 'Free', 'cancel', 0]);
