...
export const handleGoogleLogin = async (req: any, res: any) => {
    ...
    try {
        ...
        if (isFreeTier) {
            ...
            // Paid Tier Sync (Monthly)
            // We need to count properties created since the start of the current billing cycle.
            // Cycle Start = analysis_limit_reset_at - 1 month.
            try {
                if (dbUser.analysis_limit_reset_at) {
                    ...
                    const countResult = await query(
                        ...
                        [dbUser.id, cycleStartDate]
                    );
                    const actualCount = parseInt(countResult.rows[0].count || '0');

                    if (parseInt(dbUser.analysis_count) !== actualCount) {
                        console.log(`[AUTH] Correcting analysis_count for Paid user ${dbUser.email}. DB: ${dbUser.analysis_count}, Real: ${actualCount} (Since ${cycleStartDate.toISOString()})`);
                        const updateCountRes = await query(
                            'UPDATE users SET analysis_count = $1 WHERE id = $2 RETURNING analysis_count',
                            [actualCount, dbUser.id]
                        );
                ...
                console.error("[AUTH] Failed to sync analysis count for paid user", err);
            }
        }
